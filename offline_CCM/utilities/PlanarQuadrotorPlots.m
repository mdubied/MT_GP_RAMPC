classdef PlanarQuadrotorPlots
    ...
    methods
        function obj = PlanarQuadrotorPlots(options)
            arguments
                options.labelFontSize (1,1) {mustBeNumeric} = 8;
                ...
                options.alphaArea (1,1) {mustBeNumeric,mustBeNonnegative} = 0.5;
            ...
            if all(cellfun(@isempty, obj.xLabels))
                ...
                obj.xLabels{2} = 'p_y';
                obj.xLabels{3} = '\phi';
                obj.xLabels{4} = '{\dot p_x}';
                obj.xLabels{5} = '{\dot p_y}';
        ...
        function trajectories(obj,t,x,u,xTubes,uTubes,label,options)
            ...
            arguments (Input,Repeating)
                ...
                x (6,:) {mustBeNumeric}
                u (2,:) {mustBeNumeric}
                xTubes (6,:) {mustBeNumeric}
                uTubes (2,:) {mustBeNumeric}
            ...
            figure()
            % Plot states
            idx = {[1,2],[3,6],[4,5]}; % State to plot mapping
            for i = 1:3
            ...
            pos = [pos(1)+pos(3),pos(2),1-pos(1)-pos(3),pos(4)];
            axes(Position=pos, Visible='off'); hold on;
            for i = 1:N
        ...
        function exportPlot(obj, fig, varargin)
            ...
            set(fig, PaperSize=[width height]+obj.margin);           %#ok
