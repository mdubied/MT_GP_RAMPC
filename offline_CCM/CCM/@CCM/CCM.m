...
classdef CCM
    properties (GetAccess=public,SetAccess=private)
        ...
        alpha (1,1) {mustBeNumeric} = inf                                  % Factor L_G <= alpha*rho
        ...
        L (1,1) {mustBeNumeric}                                            % Lipschitz constant of g wrt M norm. Computed with the other constants
        L_G (1,1) {mustBeNumeric}                                          % Continuity constant for uncertainty 1
        G_M (1,1) {mustBeNumeric}                                          % Continuity constant for uncertainty 2
        E_M (1,1) {mustBeNumeric}                                          % Continuity constant for disturbance 
        ...
        delta_bar_0 (:,:) {mustBeNumeric}                                  % maximal \delta that satisfies constraints h_j(z_s0,v_s0) + c_j delta_bar <= 0
    ...
    methods
        function obj = CCM(sys,params,options)
            arguments
                ...
                options.alpha (:,:) {mustBeNumeric} = 0.8
                options.use_sos (1,1) logical = true
                ...
                options.n_tot (1,1) struct = struct('rccm',1E4,'rccm_check',1E5,'c_x',1E5,'c_u',1E5,'c_obs',1E5,'L_G',1E5,'G_M',1E5,'E_M',1E5,'w_bar',1E5,'delta_over',1E5,'M_under',5E3,'M_under_check',1E4)
                options.recompute (1,1) struct = struct('rccm',false,'c_x',false,'c_u',false,'c_obs',false,'L_G',false,'G_M',false,'E_M',false,'delta_over',false,'M_under',false)
            end
            % Check for casadi
            ...
            % Define additional constraint L_G <= alpha*rho
            ...
            props = ["params.F_x","params.b_x", ...
                     ...
                     "rho_des","W_idxs","monomial_degree","use_sos","alpha"];
            rccm_vars = ["rho","alpha","Y_fcn","Y_coef","W_fcn","W_coef","dW_dx","dW_dt_fcn","dW_dt_approx_fcn"];
...
%                                           options.solver_options);
        ...
        function [v, s, y0] = solve(obj,x0,theta0,options)
            ...
            s.offline.G_M = obj.G_M;
            s.offline.E_M = obj.E_M;            
            ...
            s.offline.delta_bar_0 = obj.delta_bar_0;
    ...
    methods (Access=public)
        ...
        obj = compute_L(obj,params,g_fcn,M_under_chol);
        obj = compute_L_G(obj,x_sym,u_sym,M_chol_fcn,G_fcn);
        obj = compute_G_M(obj,x_sym,u_sym,M_chol_fcn,G_fcn);
        obj = compute_E_M(obj,x_sym,d_sym,M_chol_fcn,Ed_fcn); 
